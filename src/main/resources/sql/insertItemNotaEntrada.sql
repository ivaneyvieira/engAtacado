...
INSERT INTO sqldados.iprd(invno, qtty, fob, cost, DATE, ipi, auxLong1, auxLong2, frete, seguro,
                          ...
                          bits2, bits3, bits4, s1, s2, s3, s4, s5, s6, s7, s8, prdno, grade,
                          auxChar, auxChar2, cstIcms, cstIpi, c1)
SELECT @invno, @qtty * 1000, @cost * 100 AS fob, @cost * 100, CURRENT_DATE * 1 AS DATE, 0 AS ipi
     , 0 AS auxLong1, 0 AS auxLong2, 0 AS frete, 0 AS seguro, 0 AS despesas, 0 AS freteIpi
     , 0 AS qttyRessar, 0 AS baseIcmsSubst, 0 AS icmsSubst, 0 AS icms, 0 AS discount
     , @cost * 10000 AS fob4, @cost * 10000 AS cost4, 0 AS icmsAliq, 1949 AS cfop, 0 AS auxLong3
