...
SELECT E.storeno, E.ordno, E.prdno, E.grade, qtty,
       ...
       C.no AS custno, E.empno, V.sname AS vsname, C.id_sname AS csname
...
INSERT INTO sqldados.inv(invno, vendno, ordno, xfrno, issue_date, DATE, comp_date, ipi, icm,
                         freight, netamt, grossamt,
                         subst_trib, discount, prdamt, despesas, base_ipi, aliq, cfo, nfNfno,
                         auxLong1, auxLong2, auxMoney1, auxMoney2,
                         dataSaida, amtServicos, amtIRRF, amtINSS, amtISS, auxMoney3, auxMoney4,
                         auxMoney5, auxLong3, auxLong4, auxLong5,
                         auxLong6, auxLong7, auxLong8, auxLong9, auxLong10, auxLong11, auxLong12,
                         auxMoney6, auxMoney7, auxMoney8, auxMoney9,
                         auxMoney10, auxMoney11, auxMoney12, auxMoney13, l1, l2, l3, l4, l5, l6, l7,
                         l8, m1, m2, m3, m4, m5, m6, m7, m8, weight, carrno, packages, storeno,
                         indxno, book_bits, TYPE, usernoFirst, usernoLast, nfStoreno, bits, padbyte,
                         auxShort1,
                         auxShort2, auxShort3, auxShort4, auxShort5, auxShort6, auxShort7,
                         auxShort8, auxShort9, auxShort10, auxShort11,
                         auxShort12, auxShort13, auxShort14, bits2, bits3, bits4, bits5, s1, s2, s3,
                         s4, s5, s6, s7, s8, nfname, invse, account, remarks, contaCredito,
                         contaDebito, nfNfse, auxStr1, auxStr2, auxStr3, auxStr4, auxStr5, auxStr6,
                         c1, c2)
SELECT @INVNO AS invno, vendno, ordno, 0 AS xfrno, CURRENT_DATE * 1 AS issue_date,
       ...
       0 AS auxShort2, 0 AS auxShort3, 0 AS auxShort4, 0 AS auxShort5, 0 AS auxShort6, 0 AS
         /*valor desconhecido*/ auxShort7, 0 AS /*valor desconhecido*/ auxShort8, 0 AS auxShort9,
       ...
       CONCAT(T.custno, ' ', T.csname) AS remarks, '' AS contaCredito, '' AS contaDebito,
       '' AS nfNfse, '' AS auxStr1, '' AS auxStr2, '' AS auxStr3, '' AS auxStr4, '' AS auxStr5,
       '' AS auxStr6, @DOC AS c1, '' AS c2
FROM T
WHERE @TIPO = 'E'
...
INSERT INTO sqldados.iprd(invno, qtty, fob, cost, DATE, ipi, auxLong1, auxLong2, frete, seguro,
                          ...
                          fob4, cost4, icmsAliq, cfop, auxLong3, auxLong4, auxLong5, auxMy1, auxMy2,
                          auxMy3, baseIcms, baseIpi, ipiAmt, reducaoBaseIcms, lucroTributado, l1,
                          l2, l3, l4, l5, l6, l7, l8, m1, m2, m3, m4, m5, m6, m7, m8, storeno, bits,
                          auxShort1, auxShort2, taxtype, auxShort3, auxShort4, auxShort5, seqno,
                          bits2, bits3,
...
SELECT @INVNO AS invno, qtty, cost AS fob, cost, CURRENT_DATE * 1 AS DATE, 0 AS ipi, 0 AS auxLong1
     ...
     , cost * 100 AS cost4, 0 AS icmsAliq, 1949 AS cfop, 0 AS auxLong3, 0 AS auxLong4, 0 AS auxLong5
     , 0 AS auxMy1, 0 AS auxMy2, 0 AS auxMy3, 0 AS baseIcms, 0 AS baseIpi, 0 AS ipiAmt
     , 0 AS reducaoBaseIcms, 0 AS lucroTributado, 0 AS l1, 0 AS l2, 0 AS l3, 0 AS l4, 0 AS l5
     , 0 AS l6, 0 AS l7, 0 AS l8, 0 AS m1, 0 AS m2, 0 AS m3, 0 AS m4, 0 AS m5, 0 AS m6, 0 AS m7
     , 0 AS m8, storeno, 32 AS bits, 0 AS auxShort1, 0 AS auxShort2, 0 AS taxtype, 0 AS auxShort3
     , 0 AS auxShort4, 0 AS auxShort5, 0 AS seqno, 0 AS bits2, 0 AS bits3, 0 AS bits4, 0 AS s1
     , 0 AS s2, 0 AS s3, 0 AS s4, 0 AS s5, 0 AS s6, 0 AS s7, 0 AS s8, prdno, grade, '' AS auxChar
     , '' AS auxChar2, '000' AS cstIcms, '' AS cstIpi, '' AS c1
FROM T
WHERE @TIPO = 'E';
...
SELECT @XANO AS xano, @NFNO AS nfno, custno, CURRENT_DATE * 1 AS issuedate,
       ...
       0 AS s8, @SERIE AS nfse, '' AS ship_by, '' AS vol_make, '' AS vol_kind,
       CONCAT(T.custno, ' ', T.csname) AS remarks, '' AS padbyte, '' AS print_remarks,
       '' AS remarksCancel, @DOC AS c1, '' AS c2, '' AS wshash
FROM T
WHERE @TIPO = 'S'
GROUP BY storeno, ordno;
